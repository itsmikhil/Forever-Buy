...
const Orders = () => {
  let { orders, getAllOrders, token } = useContext(AdminDataContext);
  useEffect(() => {
    if (token) {
      getAllOrders();
    }
        ...
        <div className="text-sm flex flex-col gap-2 lg:text-base lg:grid lg:grid-rows-1 lg:grid-cols-[1fr_3fr_2fr_1fr_2fr] border-1 border-gray-200 px-6 py-6">
          ...
          <div className="flex flex-col gap-3">
            {order.items.map((item) => {
              return <h1>{item.productId.name} x {item.quantity}</h1>;
            })}
            <h1>{order.deliveryDetails.firstName} {order.deliveryDetails.lastName}</h1>
            <h1>
              {order.deliveryDetails.street}, {order.deliveryDetails.city}, {order.deliveryDetails.state},
              {order.deliveryDetails.country}, {order.deliveryDetails.zipcode}
            </h1>
          </div>
          <div>
